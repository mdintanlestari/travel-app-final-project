...
const DetailTransaction = () => {
  ...
  return (
    <div className="max-w-3xl p-4 mx-auto bg-white rounded shadow">
      ...
      <p>
        <strong>Metode Pembayaran:</strong> {transaction.payment_method?.name}
      </p>
      ...
      {transaction.transaction_items.map((item) => (
        <div key={item.id} className="p-2 mb-2 bg-gray-100 rounded">
          <img src={item.imageUrls[0]} alt={item.title} className="w-32 mb-2" />
          <p>{item.title}</p>
          <p>Harga: Rp {item.price.toLocaleString("id-ID")}</p>
          <p>Diskon: Rp {item.price_discount.toLocaleString("id-ID")}</p>
